...
constexpr auto is_linear_format(vk::Format const format) {
	constexpr auto linear_formats_v =
		std::array{vk::Format::eR8G8B8A8Unorm, vk::Format::eB8G8R8A8Unorm, vk::Format::eA8B8G8R8UnormPack32};
...
void Context::close() { glfwSetWindowShouldClose(m_window.get(), GLFW_TRUE); }
...
void Context::create_device() {
	...
	for (auto const* ext : required_extensions_v) {
		...
		if (std::find_if(available_extensions.begin(), available_extensions.end(), found) ==
			...
			throw Error{"Required extension '{}' not supported by selected GPU '{}'", ext,
...
void Context::Deleter::operator()(Ptr<GLFWwindow> window) const noexcept {
	glfwDestroyWindow(window);
	glfwTerminate();
}

void Context::Deleter::operator()(Ptr<DeviceBlocker> blocker) const noexcept {
	blocker->device.waitIdle();
	std::default_delete<DeviceBlocker>{}(blocker);
}

void Context::Deleter::operator()(Ptr<DearImGui> dear_imgui) const noexcept {
